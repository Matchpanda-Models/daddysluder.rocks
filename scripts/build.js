...
function generateSchema(type, data) {
  ...
  switch (type) {
    case 'profilePage':
      ...
      const alternateNames = [config.model.username];
      if (config.model.name.toLowerCase().replace(/\s+/g, '_') !== config.model.username.toLowerCase()) {
        alternateNames.push(config.model.name.toLowerCase().replace(/\s+/g, '_'));
        alternateNames.push(config.model.name.toLowerCase().replace(/\s+/g, ' '));
      }
      return {
        ...baseSchema,
        "@type": "ProfilePage",
        ...
        "mainEntity": {
          ...
          "identifier": config.model.id || config.model.username,
          "url": config.site.url,
          "image": {
            "@type": "ImageObject",
            "url": avatarUrl,
          ...
          "description": config.model.bio || config.seo.description,
          "jobTitle": "Content Creator",
          "knowsLanguage": {
            "@type": "Language",
            "name": "Deutsch",
            "alternateName": config.site.language
          },
          "nationality": {
            "@type": "Country",
            "name": "Deutschland",
          ...
          "interactionStatistic": [{
            ...
            "userInteractionCount": data?.totalVideos || 0,
            "description": "Anzahl veröffentlichter Videos"
          }],
          "agentInteractionStatistic": [{
            "@type": "InteractionCounter",
            "interactionType": { "@type": "WriteAction" },
            "userInteractionCount": data?.totalVideos || 0,
            "description": "Anzahl erstellter Inhalte"
          }],
          ...(data?.profile ? {
            "gender": data.profile.gender || "weiblich",
